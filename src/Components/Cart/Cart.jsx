...
//   const { data, error, isLoading } = useQuery(['cart'], fetchCart);

//   const removeProductMutation = useMutation(
//     productId => axios.delete('https://ecommerce.routemisr.com/api/v1/cart/' + productId, {
//       headers: {
//         token: localStorage.getItem("token")
//       }
//     }),
...
//     ({ productId, count }) => axios.put('https://ecommerce.routemisr.com/api/v1/cart/' + productId, { count }, {
//       headers: {
//         token: localStorage.getItem("token")
//       }
//     }),
...
//           const newProducts = oldData.products.map(product =>
//             product.product._id === data.data.data.product._id ? { ...product, count: data.data.data.count } : product
//           );
//           return {
//             ...oldData,
...
//   const { products: cartProducts = [], totalCartPrice = 0, _id: cartId = '' } = data || {};
...
//   const updateProductCount = (productId, count, index) => {
...
//     {isLoading ? <div className='text-center'><i className='fas fa-spinner fa-spin fa-2x py-5 my-5 '></i></div> : null}

//     {error ? <h1 className='alert alert-danger text-center m-5'>Error: {error.message}</h1> : null}
...
//           <button onClick={clearCart} className='btn btn-danger d-block ms-auto'>Clear Cart</button>
//           {cartProducts?.map((product, index) => {
//             return <div key={product._id} className='container cart-product shadow rounded-2 my-3'>
//               <div className="row align-items-center">
//                 <div className="col-md-2">
//                   <img className='w-100' src={product.product.imageCover} alt="" />
//                 </div>
